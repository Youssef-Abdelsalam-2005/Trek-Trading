        ...
        async with self._pool.acquire() as conn:
            async with conn.transaction():
                ...
                if info["retry_count"] < info["max_retries"]:
                    backoff = self._base_backoff * (2 ** info["retry_count"])
                    result = await conn.execute(RETRY_SQL, task_id, error, backoff)
                    if result == "UPDATE 0":
                    ...
                    log.info(
                        ...
                        task_id, info["retry_count"] + 1, info["max_retries"], backoff, error,
                    )
                else:
                    result = await conn.execute(FAIL_PERMANENT_SQL, task_id, error)
                    if result == "UPDATE 0":
                        raise ValueError(f"Task {task_id} not in running state")
                    log.info(
                        "task %s permanently failed after %d retries: %s",
                        task_id, info["retry_count"], error,
                    )
